...
namespace space::world {
    ...
    void SpaceRenderer::renderLine(const Vector& from, const Vector& to, const sdl::Color& c, float gridSize) {
        ...
        _renderer->drawLine(width / 2 + 175 + from.z * gridSize, height / 2 - from.y * gridSize, width / 2+ 175 + to.z * gridSize,
                            height / 2 - to.y * gridSize);

        _renderer->drawLine(width / 2 + from.x * gridSize, height / 2 + 175 - from.z * gridSize, width / 2 + to.x * gridSize,
                            height / 2 + 175 - to.z * gridSize);
    }
